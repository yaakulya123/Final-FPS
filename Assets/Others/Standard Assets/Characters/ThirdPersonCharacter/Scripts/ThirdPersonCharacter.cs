	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
        ...
        private void OnCollisionEnter(Collision collision)
        {
            if (collision.gameObject.CompareTag("Bullet"))
            {
				...
				Vector3 deadPosition = new Vector3(transform.position.x, -5f, transform.position.z);
			 ...
			 // m_Rigidbody.isKinematic = true;
                ...
                if (random == 0)
                {
					Vector3 spawnPosition = transform.position + new Vector3(0, 6, 0);// Adjust Y position as needed
                    Instantiate(ammoBox, spawnPosition, Quaternion.identity);
                }
                else if (random == 1)
                {
                    Vector3 spawnPosition = transform.position + new Vector3(0, 6, 0); // Adjust Y position as needed
                    Instantiate(healthBox, spawnPosition, Quaternion.identity);
                }
